    ...
    private static void registerToConsul(Config config, InetSocketAddress socketAddress) {
        try {
            ...
            if (!config.consulMode.equals("agent")) {
                ...
                HostAndPort tcp = HostAndPort.fromParts(config.host, config.port);
                System.out.println(tcp.toString());
                consul.agentClient().register(config.port, tcp, 10, consulTag, serviceId);
            }
